...
function BookList () {
  ...
  useEffect(() => {
    ...
    for (const book of books) {
      data.push([book.title, Number(book.pageCount)])
    }
    setChartData(data)
  }, [books])
  ...
  const barOptions = {
    chart: {
      ...
      subtitle: 'Number of pages per book'
    }
  }
  ...
  const editBook = (rowData) => {
    ...
    setPageCount(rowData.pageCount)
    setIsDialogShown(true)
  ...
  const tableFooter = (
    <div className="flex gap-2">
      ...
      <Button label='Show Charts' icon='pi pi-chart-bar' className="p-button-secondary" onClick={() => setIsChartDialogShown(true)} />
    </div>
  )
  ...
  return (
    <div>
      ...
      <Dialog header='Visual Statistics' visible={isChartDialogShown} onHide={() => setIsChartDialogShown(false)} style={{ width: '70vw' }}>
